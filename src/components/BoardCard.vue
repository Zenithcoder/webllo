...
import draggable from 'vuedraggable'
	...
	methods:{
		createCard(listId){
			...
			axios.post('/boards/'+this.list.board_id+"/list/"+this.list.id+"/card", {name:this.cardData.name}).then(response=>{
		...
		updateCard(cardId,listId){
			axios.put('/card/'+cardId, {lists_id:listId}).then(response=>{
				console.log(response);
			});
		},
		...
		onChange(evt){
			...
			axios.patch('/card/update-all',{cards:newCards}).then(response =>{
				console.log(response);
			})
		}
	}
